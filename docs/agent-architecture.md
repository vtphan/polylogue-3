...
- The authoring workflow is: teacher provides context and target flaws (in natural language) → LLM generates a set of agents → teacher reviews and edits → LLM generates the presentation or discussion. The human curates, not authors from scratch.
...
Roles may differ by discourse mode:

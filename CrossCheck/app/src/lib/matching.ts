...
export function computeMatches(
  ...
  const locationOnly = options?.locationOnly || false;
  ...
  // Pass 1: Find green matches (correct location + correct type, or location-only in detect-only/locate mode)
  for (const ann of annotations) {
    for (const flaw of flawIndex) {
      if (
        flaw.locations.includes(ann.location.item_id) &&
        (locationOnly || flaw.flaw_type === ann.flawType) &&
